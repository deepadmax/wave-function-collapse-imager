...
class Board:
    ...
    def get_neighbours(self, ij: tuple = (0,0), N: int = 1):
    ...
    def generate(self):
        while not self.is_completed():
            ...
            neighbours = self.get_neighbours(lowest, 1)
            active = neighbours
            while len(active) > 0:
                new_active = []
                for elt in active:
                    if not self.grid[elt[0]][elt[1]].has_collapsed:
                        neighbours = self.get_neighbours(elt, 1)
                        print(neighbours)
                        neighbour_states = [self.grid[i][j] for i,j in neighbours]
                        new_states = self.pattern.match(neighbour_states)
